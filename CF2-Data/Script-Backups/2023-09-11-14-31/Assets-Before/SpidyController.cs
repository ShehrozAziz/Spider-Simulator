...
public class SpidyController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            if (Input.GetKey(KeyCode.LeftShift) && Spidycontroller.isGrounded)
            {
        ...
        if (Input.GetKeyDown(KeyCode.Q) && !IsDoingSomething)
        {
            myAnimator.SetTrigger("attack");
            IsDoingSomething = true;
            Invoke("DontDo", 1.33f);
        }
        if(Input.GetKey(KeyCode.E) && !IsDoingSomething && !IsJumping)
        {
            myAnimator.SetTrigger("pounce");
            IsDoingSomething = true;
            Invoke("DontDo", 1.5f);
        }
        if(Input.GetKeyDown(KeyCode.Z) && !IsDoingSomething)
        {
            myAnimator.SetTrigger("defend");
            IsDoingSomething = true;
            Invoke("DontDo", 1.33f);
        }
        if(Input.GetKeyDown(KeyCode.B) && !IsDoingSomething)
        {
